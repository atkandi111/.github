...
The official `openai/codex-action` runs in a job with `contents: read`, the current `:workspace` permission profile, `drop-sudo`, an ephemeral session, and the OpenAI API secret. The job captures a binary Git patch and final message as an artifact. A clean, separate job downloads the artifact, has no OpenAI secret, validates protected paths, and alone receives repository/PR/Actions write permissions.
