...
- Treat `main` as the client release channel: canary-test candidate SHAs before merge, keep normal client callers on `@main`, and enable branch protection as soon as the repository plan or visibility supports it. Until then, use pull requests, review, and the global kill switch as explicit compensating controls.
