...
def test_workflow_trust_boundaries() -> None:
    ...
    require("PIPELINE_ENABLED: ${{ inputs.pipeline_enabled }}" in agent, "publisher does not recheck the validated kill-switch input")
    require("vars.AGENT_PIPELINE_ENABLED" not in agent, "called workflow reads caller-only repository variables directly")
